import {
  fetchMeetings, fetchSessions, fetchDrivers,
  fetchLaps, fetchCarData, fetchPositions, fetchPit,
  fetchRaceControl, fetchLocation, ImportProgress, ImportStage
} from './openf1Client';
import {
  upsertMeeting, upsertSession, upsertDriver,
  upsertLap, insertCarDataBatch, upsertPosition,
  upsertPit, insertRaceControlBatch, insertLocationBatch,
  createImportStatus, updateImportStatus,
  getMeetingByKey, getSessionByKey
} from '../db/queries';
...
export async function startImport(sessionKey: number, meetingKey: number) {
  ...
  if (importJobs.has(sessionKey) && importJobs.get(sessionKey)!.status === 'running') {
...
async function runImport(sessionKey: number, meetingKey: number, importId: number) {
  console.log(`Starting import for session ${sessionKey}...`);
  
  // Step 1: Fetch and store meeting
  const meetings = await fetchWithProgress(
    sessionKey, 'fetching_meeting', 5, () => fetchMeetings(), importId
  );
  ...
  const sessions = await fetchWithProgress(
    sessionKey, 'fetching_sessions', 10, () => fetchSessions(meetingKey), importId
  );
  ...
  const drivers = await fetchWithProgress(
    sessionKey, 'fetching_drivers', 15, () => fetchDrivers(sessionKey), importId
  );
  ...
  for (let i = 0; i < driverNumbers.length; i++) {
    ...
    const laps = await fetchWithProgress(
      sessionKey, 'fetching_laps', Math.round(driverProgress), 
      () => fetchLaps(sessionKey, dn), importId
    );
  ...
  for (let i = 0; i < driverNumbers.length; i++) {
    ...
    const carData = await fetchWithProgress(
      sessionKey, 'fetching_car_data', Math.round(driverProgress),
      () => fetchCarData(sessionKey, dn), importId
    );
  ...
  for (let i = 0; i < driverNumbers.length; i++) {
    ...
    const positions = await fetchWithProgress(
      sessionKey, 'fetching_positions', Math.round(driverProgress),
      () => fetchPositions(sessionKey, dn), importId
    );
  ...
  for (let i = 0; i < driverNumbers.length; i++) {
    ...
    const pitData = await fetchWithProgress(
      sessionKey, 'fetching_pit', Math.round(driverProgress),
      () => fetchPit(sessionKey, dn), importId
    );
  ...
  const raceControl = await fetchWithProgress(
    sessionKey, 'fetching_race_control', 85,
    () => fetchRaceControl(sessionKey), importId
  );
  ...
  for (let i = 0; i < driverNumbers.length; i++) {
    ...
    const locData = await fetchWithProgress(
      sessionKey, 'fetching_location', Math.round(driverProgress),
      () => fetchLocation(sessionKey, dn), importId
    );
