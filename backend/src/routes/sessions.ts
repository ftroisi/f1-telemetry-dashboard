...
import {
  getDrivers, getLaps, getCarData, getPositions,
  getPitData, getLocationData
} from '../db/queries';
import { getFormat, sendResponse, serializeDrivers, serializeLaps, serializeCarData, serializePositions, serializePit } from '../proto/serializer';
...
router.get('/:sessionKey/drivers', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch drivers', message: err.message });
  }
});
...
router.get('/:sessionKey/laps', async (req: Request, res: Response) => {
  try {
    const sessionKey = parseInt(req.params.sessionKey, 10);
    const driverNumber = req.query.driver_number ? parseInt(req.query.driver_number as string, 10) : undefined;
    const laps = await getLaps(sessionKey, driverNumber);
    const format = getFormat(req);
    sendResponse(res, laps, format, serializeLaps);
  } catch (err: any) {
    res.status(500).json({ error: 'Failed to fetch laps', message: err.message });
  }
});
...
router.get('/:sessionKey/car-data', async (req: Request, res: Response) => {
  try {
    ...
    const carData = await getCarData(sessionKey, driverNumber, minDate, maxDate);
    const format = getFormat(req);
...
router.get('/:sessionKey/pit', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch pit data', message: err.message });
  }
});
...
router.get('/:sessionKey/location', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch location data', message: err.message });
  }
});
