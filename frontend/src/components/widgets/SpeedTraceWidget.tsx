...
export default function SpeedTraceWidget({ sessionKey, driverNumbers, lapNumber, configurable, onConfigure }: SpeedTraceWidgetProps) {
  ...
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
        <p className="text-gray-500 text-sm">No car data available</p>
      </div>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
        ...
        <YAxis yAxisId="pedals" orientation="right" tick={{ fill: '#999', fontSize: 10 }} stroke="#fbce04" domain={[0, 100]} />
        <Tooltip
        ...
        <Line yAxisId="speed" type="monotone" dataKey="speed" stroke="#1ca7e3" dot={false} strokeWidth={2} name="Speed (km/h)" />
        <Line yAxisId="pedals" type="monotone" dataKey="throttle" stroke="#27c93f" dot={false} strokeWidth={1.5} name="Throttle (%)" />
        <Line yAxisId="pedals" type="monotone" dataKey="brake" stroke="#f70814" dot={false} strokeWidth={1.5} name="Brake (%)" />
      </LineChart>
    </ResponsiveContainer>
  );
}
